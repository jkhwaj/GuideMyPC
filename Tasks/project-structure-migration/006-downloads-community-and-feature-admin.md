...
- Validation for the Community deletion-audit increment: PHP syntax checks for the two legacy routes, `CommunityAdminService`, and `community_admin_integration_test.php` passed; `php scripts/lint.php` and `composer verify` passed. The focused integration test requires a configured isolated `DB_TEST_NAME` and was not runnable in the current workspace. `php tests/helpers_test.php` returned a rendered 500 error page and needs separate environment/failure diagnosis.
