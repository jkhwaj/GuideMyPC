...
- Added `CommunityAdminService` for the legacy post and comment deletion mutations. `delete_post.php` and `delete_comment.php` retain their POST, administrator, CSRF, redirect, and no-op behavior; successful target deletions now atomically record `community.post.deleted` or `community.comment.deleted` audit events. Post audit metadata records the related like and comment deletion counts.
- Validation for the Community deletion-audit increment: PHP syntax checks for the two legacy routes, `CommunityAdminService`, and `community_admin_integration_test.php` passed; `php scripts/lint.php`, `tests/helpers_test.php`, and Composer verification passed after regenerating the local authoritative autoloader. The focused integration test requires a configured isolated `DB_TEST_NAME` and was not runnable in the current workspace.
- Extracted user lookup, duplicate-email checks, update, and deletion mutations into `UserAdminService`. `edit_user.php` retains its role validation, session refresh, and redirect behavior; `delete_user.php` retains its POST, administrator, CSRF, and self-delete protections. Successful deletions now record `user.deleted` audit events.
