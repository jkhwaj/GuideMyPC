...
- Added `tests/route_map_test.php`, a dependency-free contract check that verifies every approved legacy dispatch target is present in exactly one route-map group, maps to a root compatibility script, and cannot contain a path segment. `docs/production-deployment.md` now includes Apache and Nginx public-root examples plus public-root rollback steps. These checks do not replace the required live Apache route/private-path matrix.
- Transitional local Apache smoke checks returned `200` for `/GuideMyPC/about.php`, `404` for an unknown route, and `403` for a migration SQL path. The existing server still exposes the repository root, so a separate public-only virtual-host or Alias validation remains required before this task can complete.
