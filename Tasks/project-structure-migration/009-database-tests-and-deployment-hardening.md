...
- `scripts/verify.php` now discovers sorted `tests/*_test.php` files so new suites cannot be silently omitted. `composer run verify:fast` runs Composer validation, PHP lint, and helper tests without a database; `composer run verify` adds the guarded complete test suite. The database and release-checklist documentation record both commands and their isolated-database requirement.
- Added `database/recovery.md` with stop, backup, investigation, partial-DDL, checksum-mismatch, forward-fix, and post-recovery validation procedures. A representative failure rehearsal and backup restoration still require isolated database evidence.
