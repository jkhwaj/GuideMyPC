...
- Extracted published diagnostic-flow and node reads into `GuideMyPC\Features\Diagnostics\DiagnosticRepository`; existing `diagnostic_flow()` and `diagnostic_node()` remain compatibility delegates. Session ownership and transition/restart/back behavior remain unchanged pending dedicated database-backed coverage.
