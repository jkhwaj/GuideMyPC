...
The frontend is server-rendered HTML with responsive custom CSS and progressive vanilla JavaScript. PHP 8.2 routes use shared bootstrap, security, validation, and helper layers. MariaDB 10.4 uses `mysqli`, prepared statements, versioned migrations, and sanitized seeds. XAMPP is local-only; production requires hardened PHP hosting. Alternatives such as an API-first JavaScript rewrite were rejected because incremental PHP changes preserve the working prototype while reducing delivery risk.
